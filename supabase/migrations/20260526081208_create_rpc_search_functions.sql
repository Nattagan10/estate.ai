...
-- ============================================================
-- rpc_search_properties: paginated property search with filters
-- ============================================================
...
BEGIN
  ...
  FROM (
    SELECT r.id, r.name, r.property_type, r.province, r.district, r.neighborhood,
    ...
    WHERE
      ...
      AND (NOT COALESCE(p_has_yield,FALSE) OR
           (r.rental_yield IS NOT NULL AND r.rental_yield > 0))
    ORDER BY
      CASE WHEN p_sort_by = 'price_asc'  THEN r.price_thb    END ASC  NULLS LAST,
      CASE WHEN p_sort_by = 'price_desc' THEN r.price_thb    END DESC NULLS LAST,
      CASE WHEN p_sort_by = 'newest'     THEN r.year_built   END DESC NULLS LAST,
      CASE WHEN p_sort_by = 'yield'      THEN r.rental_yield END DESC NULLS LAST,
      r.price_thb ASC NULLS LAST
...
-- ============================================================
-- rpc_fetch_map_pins: lightweight map pin fetch (max 500 rows)
-- ============================================================
...
BEGIN
  ...
  FROM (
    ...
    WHERE
      ...
      AND (NOT COALESCE(p_has_yield,FALSE) OR
           (r.rental_yield IS NOT NULL AND r.rental_yield > 0))
    ORDER BY r.price_thb ASC NULLS LAST
    LIMIT 500
  ) q;
